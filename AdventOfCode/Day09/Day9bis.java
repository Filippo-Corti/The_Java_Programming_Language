...
public class Day9bis {
    ...
    public static long findPrevNumber(ArrayList<Long> l) {
        if (l.stream().allMatch(entry -> entry == 0)) {
            return 0;
        }
        return l.get(0) - findPrevNumber(calcDifferences(l));
    }

    public static void main(String[] args) {
        ArrayList<ArrayList<Long>> input = parseInput();

        System.out.println(input.stream().mapToLong((list) -> findPrevNumber(list)).sum());
    }
