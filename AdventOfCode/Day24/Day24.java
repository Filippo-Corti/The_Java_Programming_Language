...
public class Day24 {
    ...
    public static ArrayList<Hailstone> parseInput() {
        ...
        while (s.hasNextLine()) {
            ...
            hailstones.add(new Hailstone(findStraightLineFromTwoPoints(x1, y1, x2, y2), new Point(x1, y1), new Speed(Long.parseLong(in[3].trim()), Long.parseLong(in[4].trim()))));
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < hailstones.size(); i++) {
           for (int j = i+1; j < hailstones.size(); j++) {
                ...
                if (intersection.x >= MIN && intersection.x <= MAX && intersection.y >= MIN && intersection.y <= MAX && h1.isReachable(intersection) && h2.isReachable(intersection)) {
                    c++;
                }
           } 
        }
